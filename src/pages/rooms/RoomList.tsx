        ...
        data?.result?.map((item) => {
          return (
            <Fade triggerOnce direction="up" key={item._id}>
              <div className="card md:card-side bg-white shadow-xs overflow-hidden">
                <div className="w-full md:w-auto mb-5 md:mb-0">
                  <div className="h-full">
                    <img src={item.imageUrl} alt={item.name} className="h-full w-full object-cover" />
                  </div>
                </div>
                <div className="w-full md:w-[380px] flex flex-col px-5 pb-8 md:p-8">
                  <Link to={`/rooms/${item._id}`}>
                    <h3 className="text-3xl font-bold mb-2 hover:text-primary-100 transition-all">{item.name}</h3>{' '}
                  </Link>
                  <p className="font-semibold mb-5 md:mb-12">{item.description}</p>
                  <div className="flex space-x-4 mb-10">
                    <div className="flex flex-col justify-center items-center border border-primary-40 rounded-lg p-4 w-24 h-24">
                      <div className="mb-2">
                      ...
                      <p className="whitespace-nowrap">{item.areaInfo}</p>
                    </div>
                    <div className="flex flex-col justify-center items-center  border border-primary-40 rounded-lg p-4 w-24 h-24">
                      <div className="mb-2">
                        <svg width="24" height="25" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                      ...
                      <p className="whitespace-nowrap">{item.bedInfo}</p>
                    </div>
                    <div className="flex flex-col justify-center items-center  border border-primary-40 rounded-lg p-4 w-24 h-24">
                      <div className="mb-2">
                        <svg width="24" height="25" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                      ...
                      <p className="whitespace-nowrap">{`2-${item.maxPeople}人`}</p>
                    </div>
                  </div>
                  <div className="mb-6 md:mb-10 block w-full h-[1px] bg-gradient-to-r from-primary-100 to-white"></div>
                  <div className="flex justify-between">
                    <p className="text-3xl text-primary-100">NT$ {item.price}</p>
                    <Link to={`/rooms/${item._id}`}>
                      <MdArrowForward className="w-8 h-8 text-primary-100 hover:text-primary-120" />
                    </Link>
                  </div>
                </div>
              </div>
